...
function LoadForms(): Array<Entity> {
    ...
    return chain(formsToLoad).groupBy(it => it["entityId"]).map((forms: Array<any>, key) => {
        return {
            id: key as string,
            entityTypeId: forms[0]["entityTypeId"],
            profile: chain(forms).filter(form => form["id"] === key).map(form => toForm(form)).value()[0],
            visits: chain(forms)
                .filter((form) => form["id"] !== key && (form["orgUnitId"] === localHealthFacility?.id || form?.values?.org_unit_id === localHealthFacility?.id))
